...
proc main() =
  ...
  # Create the node for our master list and insert into root.
  ...
  # vertical space.
  l.setAlign(masterList, {AlignTop, AlignBottom}) # {AlignTop, AlignBottom} = fill parent's vertical space
  ...
  l.setAlign(contentView, {AlignLeft, AlignTop, AlignRight,
      AlignBottom}) # Use `Align` enum combination: {AlignLeft, AlignTop, AlignRight, AlignBottom} = fill parent's horizontal + vertical space

  # Normally at this point, we would probably want to create nodes for our
  # master list and our content view and insert them. This is just a simplified
  # example, so let's move on to finishing up.

  # This does all of the actual calculations(computes size and absolute position for all nodes in the subtree).
  l.compute(root)

  # Export layout file for visual debugging.
  writeFile("example.json", l.dumpJson(root))

  # Now we can get the calculated size of our nodes as 2D rectangles. The four
  ...
  # reallocation occurs.
  #
  # However, while it's true that you could manually update the existing nodes
  ...
  # invalidation, and a context with thousands of nodes will probably still only
  ...
  # nodes, etc. The reason we don't create a new context from scratch is that we
